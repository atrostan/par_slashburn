...
//            }
        ...
        if (component_sizes.size() == 1) { // after hub removal, if graph is a single CC, recur
            clear();
            n_iters++;
            continue;
        }
...
void SlashBurn::get_set(pvector<uint64_t> &p, std::set<uint64_t> &s) {
//    pvector<uint64_t> copy(p.begin(), p.end());
//    ips4o::parallel::sort(
//            copy.begin(),
//            copy.end()
//    );
//    uint64_t i = 0;
//    for (uint64_t n = 0; n < p.size(); ++n) {
//        if (copy[i] == n) {
//            ++i;
//        }
//        else {
...
//    fmt::print("parallel transforming: \n");
//    std::transform(
...
//#pragma omp parallel for schedule(static)
        for (uint64_t i = 0; i < n; i++) {
            ...
            // todo: hacky; should be exactly 0 for last decrement
//            if (degrees[vid] < decrement) degrees[vid] = 0;
            degrees[vid] -= decrement;
            // if we haven't already deactivated this vertex and it's now a singleton,
            // add it to a parallel hash set so that we could deactivate produced singletons sequentially
            if (degrees[vid] == 0 && !vertex_inactive(vid)) {
                singletons.insert(vid);
            }
        }
//        fmt::print("singletons: {}\n", singletons);
    fmt::print("singletons.size(): {}\n", singletons.size());
    fmt::print("active_vertex_set_size: {}\n", active_vertex_set_size);
    if (singletons.size() > active_vertex_set_size) {
        fmt::print("singletons: {}\n", singletons);
        std::set<uint64_t> test_set;
        get_set(perm, test_set);
        uint64_t missing = 0;
        for (uint64_t i = 0; i < num_nodes; ++i ){
            if (perm[i] != 0xFFFFFFFFFFFFFFFF) {
                if (singletons.find(perm[i]) != singletons.end()) {
                    fmt::print("perm[i]: {}\n", perm[i]);
                }
            } else {
                ++missing;
        ...
        fmt::print("missing: {}\n", missing);
//        fmt::print("test_set: {}\n", test_set);
        fmt::print("test_set.size(): {}\n", test_set.size());
        assert(false);
    }
    //todo - below done sequentially - under assumption that produced singletons are rare
    // may be worth to parallelize;
    // deactivate all produced singletons, place them in the back of the permutation array, and update
    // spokes_end accordingly
    // test how many singletons are roughly produced in this manner for each iteration on a sizeable graph
    for (const auto &vid: singletons) {
        perm[spokes_end - 1] = vid;
        bmap.set_bit_atomic(vid);
...
//            ++n_incident_edges;
        ...
        // u has been marked, so zero its degree
        decrements[u] = degrees[u];
...
//            if (comps.contains(cid)) { comps[cid].insert(i); }
            bool cid_in_comps = comps.modify_if(
                    cid,
...
//            else {
            if (!cid_in_comps) {
                comps.try_emplace_l(
                        cid,
                        [&i](map_t::value_type &val) {
                            val.second.insert(i);
                        }, // called only when cid already present in map
...
//    uint64_t comp_start_idx = spokes_end;
    uint64_t comp_start_idx = hub_start;
...
//    fmt::print("perm_start_idcs: {}\n", perm_start_idcs);
//    get_active_vertex_set();
//    fmt::print("active_vertex_set before: {}\n", active_vertex_set);
//    fmt::print("comp_vec: {}\n", comp_vec);
    ...
    uint64_t avss_decrement = 0;
    get_active_vertex_set();
    std::unordered_set<uint64_t> active_vset(active_vertex_set.begin(), active_vertex_set.end());
    fmt::print("active_vset.size(): {}\n", active_vset.size());
#pragma omp parallel for schedule(static, 1) reduction(+:avss_decrement)
    for (uint64_t i = 0; i < comp_vec.size(); ++i) {
        ...
        for (uint64_t j = perm_start_idcs[i]; j < perm_start_idcs[i] + comp.size(); ++j) {
            ...
            // invalidate vertices in final component for a final check for singletons in active_vertex_set
            bmap.set_bit_atomic(comp[k]);
            ++k;
            ++avss_decrement;
        }
    }

    std::unordered_set<uint64_t> comp_set;
    for (const auto &v: comp_vec) {
        std::copy(v.begin(), v.end(), std::inserter(comp_set, comp_set.end()));
    }


    fmt::print("avss_decrement: {}\n", avss_decrement);
    
    active_vertex_set_size -= avss_decrement;
    spokes_end -= active_vertex_set_size;
    // at this point, the active_vertex_set should strictly contain singletons
    // place them in the remaining slots in perm
    get_active_vertex_set();
    fmt::print("active_vertex_set after: {}\n", active_vertex_set);

    fmt::print("comp_set.size(): {}\n", comp_set.size());

//    return;
//    fmt::print("hub_increment: {}\n", avss_decrement);
//#pragma omp parallel for schedule(static)
    for (uint64_t i = 0; i < active_vertex_set_size; ++i) {
        fmt::print("finally placing: {} with degree: {}\n", active_vertex_set[i], degrees[active_vertex_set[i]]);
        perm[spokes_end + i] = active_vertex_set[i];
    }
}
...
//		std::sort(dpl::execution::par_unseq, spokes_vec[spokes_idx].begin(), spokes_vec[spokes_idx].end());
    ...
    // compute a partial sum so that each spoke corresponds to an index into the final perm array
    std::vector<uint64_t> start_idcs(spokes_vec.size());
    std::transform(
            dpl::execution::par_unseq,
            spokes_vec.begin(),
            spokes_vec.end(),
    ...
    uint64_t n_spokes_to_remove = start_idcs[start_idcs.size() - 1] + \
...
#pragma omp parallel for schedule(static, 1) reduction(+:avss_decrement)
    for (uint64_t i = 0; i < spokes_vec.size(); ++i) {
        ...
        uint64_t k = 0;
        for (uint64_t j = start_idcs[i]; j < start_idcs[i] + spoke.size(); ++j) {
            uint64_t l = j; // index into spokes_to_remove vec
...
//			perm[j] = spoke[k];
    ...
    par_decrement();
    uint64_t offset = spokes_end - n_spokes_to_remove;
#pragma omp parallel for schedule(static)
    for (uint64_t i = 0; i < n_spokes_to_remove; ++i) {
        perm[i + offset] = spokes_to_remove[i];
    }

//	remove_from_active_vertex_set(spokes_to_remove);
//    fmt::print("spokes_to_remove: {}\n", spokes_to_remove);
//    fmt::print("n_spokes_to_remove: {}\n", n_spokes_to_remove);
//
...
#pragma omp parallel for schedule(static)
    for (uint64_t i = 0; i < all_counts.size(); ++i) {
        ...
        component_sizes.try_emplace_l(cid,
                                      [&n_vs_in_component](
                                              hash_t::value_type &val) { val.second += n_vs_in_component; },
                                      n_vs_in_component);
    }
    ...
    fmt::print("SB Iteration: {} [k: {}, gcc_size: {}, avss: {}, start: {}, end: {}] \n",
               n_iters, k, gcc_size, active_vertex_set_size, hub_start, spokes_end);
    if (gcc_size <= k) {
//        fmt::print("here\n");
...
NodeID SlashBurn::SampleFrequentElement(int64_t num_samples = 1024) {
    ...
    get_active_vertex_set();
    fmt::print("active_vertex_set.size() ?= active_vertex_set_size; {} ?= {} \n", active_vertex_set_size, active_vertex_set.size());
    if (active_vertex_set.size() != active_vertex_set_size) {
        assert(false);
    }
    ...
    for (NodeID i = 0; i < num_samples; i++) {
        NodeID n = active_vertex_set[distribution(gen)];sample_counts[comp[n]]++;
    }
