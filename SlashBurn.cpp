...
#pragma omp parallel for schedule(dynamic, 16384)
	for (uint64_t i = 0; i < vertices_to_decrement.size(); i++) {
		...
		// iterate over out-neighbourhoods, ignoring inactive vertices, to compute degree decrements
		...
		decrements.try_emplace_l(u,
		                         [this, u](hash_t::value_type &val) { val.second = degrees[u]; },
		                         degrees[u]);
	}
}
...
#pragma omp parallel for schedule(static)
	for (uint64_t i = 0; i < comp.size(); ++i) {
		uint64_t cid = comp[i];
		if (vertex_inactive(i)) { continue; } // vertex has already been assigned a new id
		else {
...
void SlashBurn::print_progress() {
	fmt::print(
		"SB Iteration {:<7} [k: {:^15}| GCC size: {:^20}| Active Vertex Set Size: {:^20}| start: {:^20}| end: {:^20}] \n",
...
#pragma omp parallel for
	for (uint64_t i = 0; i < g.num_nodes(); ++i) { vids[i] = i; }
	populate_degrees();
}
